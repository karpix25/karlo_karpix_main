...
class ContentOrchestratorAgent:
    ...
    async def summarize(self, text: str) -> str:
        ...
        prompt = f"""
        Проанализируй пост из Telegram и создай краткое описание на русском языке.
        
        Формат ответа (только текст, без заголовков разделов и нумерации):
        1. Первая строка: Яркая, цепляющая фраза (Hook).
        2. Пустая строка.
        3. Остальной текст: 2-3 предложения, раскрывающие суть и конкретную пользу.
        
        Правила:
        - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать слова "Хук", "Hook", "Заголовок", "Суть", "Essence" или "Описание".
        - НЕ используй цифры 1, 2 для списков.
        - Пиши сразу по делу.
